...
class LinkMessaging : public Messaging {
 public:
  ...
  void send(const void* data, size_t size) override {
    connect();
    SocketOutputStream& out = this->out();
 ...
 private:
  void connect() {
    ...
    while (true) {
      ...
      if (status == LinkStatus::kConnected ||
          status == LinkStatus::kConnecting) {
  ...
  roo_transport::SocketInputStream& in() {
    roo::lock_guard<roo::mutex> guard(mutex_);
    return link_.in();
  }

  roo_transport::SocketOutputStream& out() {
    roo::lock_guard<roo::mutex> guard(mutex_);
    return link_.out();
  }
