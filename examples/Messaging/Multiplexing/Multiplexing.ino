...
// This example demonstrates how to multiplex several link messaging channels
// over a single underlying messaging transport. (For entry example, see
// Simple.ino).
//
// Messaging transport can be quite limiting as it allows only a single
// bidirectional messaging stream. Multiplexing allows establishing multiple
// independent messaging connections over a single underlying messaging
// transport. Each channel behaves as a separate messaging instance, and
// implements the same base interface.
//
// In this example, we use two messaging channels. For simplicity, they are used
// in a 'half-duplex' mode: one channel is used for a 'client' to send the
// messages to the 'server', while the other channel is used for the opposite
// direction.
...
// Create the 'baseline' server link messaging layer over the reliable serial
// transport.
LinkMessaging server_messaging(server_serial, kMaxPayloadSize);

// Create the multiplexed messaging layer over the link messaging.
MuxMessaging server_mux_messaging(server_messaging);

// Create two channels over the multiplexed messaging layer.
MuxMessaging::Channel server_in(server_mux_messaging, 1);
MuxMessaging::Channel server_out(server_mux_messaging, 2);

// Declare the receiver for the 'server_in' channel.
...
void server() {
  ...
  // Note that we don't register the receiver for the 'server_out' channel, as
  // in this example it is used only for sending messages from server to client.
  server_in.setReceiver(server_receiver);

  // Start the receive loop.
  server_messaging.begin();

  // Send a periodic message every second on the 'server_out' channel.
  // Note: don't send anything on the 'server_messaging' transport directly, as
  // it would interfere with the multiplexing protocol.
  while (true) {
    roo_io::byte buf[32];
    roo_io::MemoryOutputIterator it(buf, buf + 32);
    roo_io::WriteString(it, "Hello!");
    server_out.send(buf, it.ptr() - buf);
    delay(1000);
  }
}
...
// Create the multiplexed messaging layer over the link messaging.
MuxMessaging client_mux_messaging(client_messaging);

// Create two channels over the multiplexed messaging layer, using the same IDs
// as we used on the server side. Because of these matching IDs, the 'client_in'
// channel corresponds to the 'server_out' channel, and vice versa.
MuxMessaging::Channel client_out(client_mux_messaging, 1);
MuxMessaging::Channel client_in(client_mux_messaging, 2);
...
// This is our client's entry point.
void client() {
#if defined(ESP_PLATFORM)
  ...
  // Note that we don't register the receiver for the 'server_out' channel, as
  // in this example it is used only for sending messages from server to client.
  client_in.setReceiver(client_receiver);
  ...
  while (true) {
    ...
    client_out.send(buf, it.ptr() - buf);
    delay(1000);
  }
}
